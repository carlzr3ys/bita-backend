...
error_log("Get My Uploads API - User ID: " . $userId);

$conn = getDBConnection();

if (!$conn) {
    ob_end_clean();
    error_log("Get My Uploads API - Database connection failed");
...
$sql .= ", mc.name as week_name, mc.level as week_level,
        ...
        ORDER BY " . ($hasPinnedColumn ? "mf.is_pinned DESC, " : "") . "mf.created_at DESC";
...
if (!$executeResult) {
    ...
    $stmt->close();
    $conn->close();
    error_log("Get My Uploads API - Execute failed: " . $error);
...
error_log("Get My Uploads API - Rows found: " . $rowCount);
error_log("Get My Uploads API - Files array count: " . count($files));

$stmt->close();
$conn->close();

error_log("Get My Uploads API - Returning " . count($files) . " files");
